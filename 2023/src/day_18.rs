...
came back. However, to catch up with the large backlog of parts requests, the factory will also need a large
supply of lava for a while; the Elves have already started creating a large lagoon nearby for this purpose.
...
When viewed from above, the above example dig plan would result in the following loop of trench (#) having
been dug out from otherwise ground-level terrain (.):
...
The Elves are concerned the lagoon won't be large enough; if they follow their dig plan, how many cubic meters
...
After a few minutes, someone realizes what happened; someone swapped the color and instruction parameters
when producing the dig plan. They don't have time to fix the bug; one of them asks if you can extract the
...
Convert the hexadecimal color codes into the correct instructions; if the Elves follow this new dig plan, how
many cubic meters of lava could the lagoon hold?
...
mod test {
    ...
    fn part_1(input: &str) -> isize {
        // Everytime we go right, we subtract the infinite area to the
        // north, everytime we go left, we add the infinite area
        // to the north. In addition to this general idea, because the
        // boundary is also included in the area, we need to add up
        // the blocks when traversing downwards (NOT upwards). And
        // finally add 1 because we never counted the starting square.
        1 + input
            .trim()
            .lines()
    ...
    fn t_part_1() {
        ...
        assert_eq!(part_1(INPUT), 67891);
    }

    #[test]
    fn t_part_2() {
        assert_eq!(part_2(EXAMPLE), 952408144115);
        assert_eq!(part_2(INPUT), 94116351948493);
    }
