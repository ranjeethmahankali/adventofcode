...
With your help, the hot springs team locates an appropriate spring which launches you neatly and precisely up
to the edge of Lava Island.
...
ignored; every other column has a reflected column within the pattern and must match exactly: column 2
...
row 8, but since that's not in the pattern, row 1 doesn't need to match anything. The remaining rows match: row
...
exists. Row 7 has no corresponding reflected row and can be ignored, but every other row matches exactly: row
...
mod test {
    ...
    fn part_1(input: &str) -> usize {
        fn symmetry_rows(grid: &[&str]) -> usize {
            match (1..grid.len()).find(|&i| {
                let left = &grid[..i];
                let right = &grid[i..];
        ...
        fn symmetry_cols(grid: &[&str]) -> usize {
            match (1..grid[0].len()).find(|&i| {
                let (lbegin, lend) = (0, i);
                let (rbegin, rend) = (i, grid[0].len());
                let len = usize::min(rend - rbegin, lend - lbegin);
                let (lbegin, lend) = (i - len, i);
                let (rbegin, rend) = (i, i + len);
                grid.iter().all(|row| {
                    row[lbegin..lend]
                        .chars()
                        .zip(row[rbegin..rend].chars().rev())
                        .all(|(l, r)| l == r)
                })
            }) {
                Some(i) => i,
        ...
        input
            ...
            .map(|lines| symmetry_cols(lines) + 100 * symmetry_rows(lines))
            .sum()
    }
    ...
    fn part_2(input: &str) -> usize {
        fn symmetry_rows(grid: &[&str]) -> usize {
            match (1..grid.len()).find(|&i| {
                ...
                left.iter()
                    .zip(right.iter().rev())
                    .map(|(&l, &r)| l.chars().zip(r.chars()).filter(|(a, b)| a != b).count())
                    .sum::<usize>()
                    == 1
            }) {
                Some(i) => i,
    ...
    fn t_part_1() {
        ...
        assert_eq!(part_1(INPUT), 28895);
    }

    #[test]
    fn t_part_2() {
        assert_eq!(part_2(EXAMPLE), 400);
        assert_eq!(part_2(INPUT), 31603);
    }
