...
fn part_1() {
    ...
    let inputlines = _INPUT.trim().lines();
    let mut cards = vec![
    ...
    let answer: usize = handbids
        .iter()
        .enumerate()
        .map(|(i, (_, _, bid))| (i + 1) * bid)
        .sum();
    assert_eq!(answer, 249483956);
