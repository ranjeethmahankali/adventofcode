...
Maybe you can learn something by looking at the value of the X register throughout execution. For now,
consider the signal strength (the cycle number multiplied by the value of the X register) during the 20th cycle
and every 40 cycles after that (that is, during the 20th, 60th, 100th, 140th, 180th, and 220th cycles).
...
mod test {
    ...
    fn part_1(input: &str) -> i64 {
        input
            ...
            .lines()
            .fold(
                (0i64, 0i64, 20i64, 1i64),
                |(mut strength, mut finished, mut target, x), line| {
                    assert!(finished < target);
                    let (val, ncycles) = if let Some(val) = line.strip_prefix("addx ") {
                        (val.parse().unwrap(), 2)
                    } else {
                        (0, 1)
                    };
                    ...
                    (strength, finished, target, x + val)
                },
            )
            .0
    }
    ...
    fn part_2(input: &str) -> String {
        let (pixels, ..) = input.trim().lines().fold(
            (['.'; 240], 0usize, 1i64),
            |(mut display, mut finished, x), line| {
                let (val, ncycles) = if let Some(val) = line.strip_prefix("addx ") {
                    (val.parse().unwrap(), 2)
                } else {
                    (0, 1)
                ...
                for cycle in 0..ncycles {
                    let pix = (finished + cycle) % display.len();
                    display[pix] = if i64::abs(x - ((pix % 40) as i64)) < 2 {
                        '#'
                    } else {
                        '.'
                    };
    ...
    fn t_part_1() {
        ...
        assert_eq!(part_1(INPUT), 12980);
    }

    #[test]
    fn t_part_2() {
        assert_eq!(
            part_2(EXAMPLE).as_str(),
            "
##..##..##..##..##..##..##..##..##..##..
###...###...###...###...###...###...###.
####....####....####....####....####....
